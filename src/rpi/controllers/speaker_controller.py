...
class SpeakerController:
    ...
    def toggle_amplifier(self):
        if self.tpa.amplifier_shutdown:
            self.tpa.amplifier_shutdown = False
        else:
            self.tpa.amplifier_shutdown = True

    def toggle_speaker(self):
        if (self.tpa.speaker_enable_r is True) and (self.tpa.speaker_enable_l is True):
    ...
    def set_attack_time(self, attack_time: int):
        assert 1 <= attack_time <= 63, "Invalid attack time (must be between values 1~63)"
    ...
    def set_fixed_gain(self, fixed_gain: int):
        if self.tpa.compression_ratio == self.tpa.COMPRESSION_1_1:
            assert 0 <= fixed_gain <= 30, "Compression disable. Invalid fixed gain (must be between values 0~30)"
        else:
            assert -28 <= fixed_gain <= 30, "Compression enabled. Invalid fixed gain (must be between values -28~24)"
        self.tpa.fixed_gain = fixed_gain

    def set_hold_time(self, hold_time: int):
        assert 1 <= hold_time <= 63, "Invalid hold time (must be between values 1~63)"
        self.tpa.hold_time = hold_time

    def set_max_gain(self, max_gain: int):
        assert 18 <= max_gain <= 30, "Invalid max gain (must be between values 18~30)"
    ...
    def set_noise_gate_threshold(self, noise_gate_threshold):
        ...
        assert noise_gate_threshold in [self.tpa.NOISE_GATE_1, self.tpa.NOISE_GATE_4, self.tpa.NOISE_GATE_10,
                                        self.tpa.NOISE_GATE_20], "Invalid noise gate threshold"
    ...
    def set_output_limiter_level(self, level: float):
        ...
        """
        assert -6.5 <= level <= 9, "Invalid output limiter level (must be between values -6.5~9)"
    ...
    def set_release_time(self, release_time: int):
        ...
        """
        assert 1 <= release_time <= 63, "Invalid release time (must be between values 1~63)"
        self.tpa.release_time = release_time

    def reset(self):
        if self.tpa.reset_Fault_l:
            self.tpa.reset_Fault_l = False
        if self.tpa.reset_fault_r:
            self.tpa.reset_fault_r = False
        if self.tpa.reset_thermal:
