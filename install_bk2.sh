...
install_eymo() {
  ...
  if [ ! -f /home/"$CURRENT_USER"/eymo_core.zip ]; then
    printf "\033[0;31m[ERROR]\033[0m The eymo_core.zip file does not exist.\n"
    exit 1
  fi

	clear
	unzip /home/"$CURRENT_USER"/eymo_core.zip -d $APP_PATH

	clear
	printf "The zip file has been extracted successfully.\n"
	read -p "Do you want to remove the eymo_core.zip file? [Y/n] " -n 1 -r
	echo
	if [[ $REPLY =~ ^[Yy]$ ]]; then
    rm -f /home/"$CURRENT_USER"/eymo_core.zip
  fi
...
uninstall_eymo() {
    ...
    rm -f /etc/systemd/system/eymo.service
		...
		apt-get remove --purge python3 python3-pip python3-rpi.gpio python3-opencv python3-virtualenv -y
